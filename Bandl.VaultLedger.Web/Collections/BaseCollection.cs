...
namespace Bandl.Library.VaultLedger.Collections
{
    ...
    public class BaseCollection : CollectionBase
	{
        ...
        public void ResetErrors()
        {
            ...
            HasErrors = false;
            // If the consituent objects are Details descendents, clear the error strings
            if (InnerList.Count != 0 && InnerList[0] is Details)
                for (int i = 0; i < this.InnerList.Count; i++)
                    ((Details)this.InnerList[i]).RowError = String.Empty;
        }
    }
}
