...
{
    /// <summary>
    /// Summary description for DomesticReceiveParser.
    /// </summary>
    public class DomesticReceiveParser : Parser, IParserObject
    {
        public DomesticReceiveParser() {}
        ...
        /// Nothing
        /// </returns>
        private void ReadListItems(StreamReader sr, ref SendListItemCollection slic)
        {
            string fileLine;
            // Read through the file, collecting items
            while ((fileLine = sr.ReadLine()) != null)
                if (fileLine.Trim().Length != 0)
                    slic.Add(new SendListItemDetails(fileLine.Substring(0, fileLine.IndexOf(' ')), String.Empty, String.Empty, String.Empty));
        }
        ...
        public override void Parse(byte[] fileText, out SendListItemCollection sendCollection, out ReceiveListItemCollection receiveCollection)
        {
            ...
            // receive lists can only create send lists as output.
            using (StreamReader sr = new StreamReader(ms))
            {
                if (MovePastListHeaders(sr))
                    ReadListItems(sr, ref sendCollection);
            }
        }
    }
}
