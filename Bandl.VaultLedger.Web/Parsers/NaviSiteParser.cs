...
namespace Bandl.Library.VaultLedger.Parsers
{
    ...
    /// </summary>
    public class NaviSiteParser : Parser, IParserObject
    {
        private ListTypes t1;
        private String accountName = String.Empty;

        public NaviSiteParser() { this.t1 = ListTypes.Send; }
        ...
        private bool MovePastListHeaders(StreamReader r1)
        {
            ...
            while ((s1 = r1.ReadLine()) != null)
            {
                if (s1.Trim().Length != 0)
                {
                    this.accountName = s1.Substring(0, s1.IndexOf(' '));
                    this.t1 = s1.ToUpper().IndexOf("SHIP") != -1 ? ListTypes.Send : ListTypes.Receive;
                    // Skip next line
                    r1.ReadLine();
                    // Return
                    return true;
        ...
        private void ReadListItems(StreamReader r1, ref IList il, ref ArrayList s1, ref ArrayList a1, ref ArrayList p1)
        {
            ...
            while ((y1 = r1.ReadLine()) != null)
            {
                ...
                {
                    String[] y2 = y1.Split(e1);
                    // Anything?
                    if (y2[0].Length == 0) continue;
                    // Add the item
                    if (il is ReceiveListItemCollection)
                    {
                        il.Add(new ReceiveListItemDetails(y2[0], String.Empty));
                    }
                    else if (this.EmployReturnDate == false || y2[1].Trim().Length == 0)
                    {
                        il.Add(new SendListItemDetails(y2[0], String.Empty, String.Empty, String.Empty));
                    }
                    else
                    {
                        String x1 = DateTime.ParseExact(y2[1], "M/d/yyyy", CultureInfo.InvariantCulture).ToString("yyyy-MM-dd");
                        il.Add(new SendListItemDetails(y2[0], x1, String.Empty, String.Empty));
                    }
