...
{
    /// <summary>
    /// Parses a PSECU TSM report
    /// </summary>
    public class PSECUParser : Parser, IParserObject
    {
        public PSECUParser() {}
        ...
        private ListTypes MovePastListHeaders(StreamReader r)
        {
            bool b1 = false;
            String s1 = null;
            ListTypes x1 = ListTypes.Receive;

            while (b1 == false && (s1 = r.ReadLine()) != null)
            {
                if (s1.ToUpper().IndexOf("SEND OFFSITE") != -1)
                {
                    x1 = ListTypes.Send;
                    b1 = true;
                }
                else if (s1.ToUpper().IndexOf("RETURN FROM OFFSITE") != -1)
                {
                    x1 = ListTypes.Receive;
                    b1 = true;
                }
            }
            // List type undiscovered?
            if (b1 == false)
            {
            ...
            {
                if (s1.IndexOf('-') != -1 && s1.Replace('-', ' ').Replace('\t', ' ').Trim().Length == 0)
                {
                    break;
                }
            }
