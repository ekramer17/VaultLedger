...
namespace Bandl.Library.VaultLedger.Parsers
{
    ...
    /// </summary>
    public class UMBBankDisasterParser : Parser, IParserObject
    {
        public UMBBankDisasterParser() {}
        ...
        private void ReadListItems(StreamReader sr, ref DisasterCodeListItemCollection li)
        {
            ...
            while ((fileLine = LeftTrim(sr.ReadLine())) != null)
            {
                ...
                else if (fileLine.IndexOf("TOTAL VOLSERS") != -1)
                {
                    break;
                }
                else
                {
                    ...
                    fileLine = fileLine.Substring(fileLine.IndexOfAny(c)).Trim();
                    serialNo = fileLine.Substring(0, fileLine.IndexOfAny(c));
                    // Get the note if necessary
                    if (DataSetNoteAction != "NO ACTION")
                    {
                        fileLine = fileLine.Substring(fileLine.IndexOfAny(c)).Trim();
                        dataNote = fileLine.Substring(0, fileLine.IndexOfAny(c));
                    }
                    // Add to item collection
                    li.Add(new DisasterCodeListItemDetails(serialNo, String.Empty, dataNote));
                }
            }
        }
        /// <summary>
