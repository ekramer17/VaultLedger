...
{
	/// <summary>
	/// Summary description for new_compare_receive_list_batch_file_step_one
	/// </summary>
	public class new_compare_receive_list_batch_file_step_one : BasePage
	{
        ...
        protected override void Event_PageLoad(object sender, System.EventArgs e)
        {
            ...
            else
            {
                ...
                TabPageDefault.Update(TabPageDefaults.ReceiveReconcileNewFile, Context.Items[CacheKeys.Login], 2);
                // Create page caption
            ...
            else
            {
                try
                {
                    ...
                    if (Encoding.UTF8.GetString(inputFile).StartsWith("VAULTLEDGER IMATION RFID ENCRYPTED XML DOCUMENT"))
                    {
                        ...
                        ReceiveListScanItemCollection rlis = new ReceiveListScanItemCollection();
                        // Get the scan items
                        if ((rlis = Imation.GetReceiveScanItems(inputFile)).Count != 0)
                        {
                            // Create the scan
                            ReceiveList.CreateScan(pageList.Name, this.txtFileName.Text.Trim(), ref rlis);
                    ...
                    {
                        // Create the scan
                        ReceiveList.CreateScan(pageList.Name, this.txtFileName.Text.Trim(), inputFile);
