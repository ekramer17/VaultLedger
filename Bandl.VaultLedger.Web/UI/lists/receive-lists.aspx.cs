...
{
	/// <summary>
	/// Summary description for receive_lists.
	/// </summary>
	public class receive_lists : BasePage
	{
        private int pageNo;
        ...
        protected override void Event_PageLoad(object sender, System.EventArgs e)
        {
            ...
            this.SetControlAttr(txtPageGoto, "onkeyup", "digitsOnly(this);");
            // Make sure that the enter key changes the hidden field
            this.SetControlAttr(txtPageGoto, "onkeypress", "if (keyCode() == 13) getObjectById('enterGoto').value = this.value;");
        ...
        protected override void Event_PagePreRender(object sender, System.EventArgs e)
        {
            ...
            switch (CustomPermission.CurrentOperatorRole())
            {
				...
				case Role.VaultOps:
					...
					// Remove link to DisasterRecovery
					break;
                case Role.Auditor:
                case Role.Viewer:
                default:
        ...
        private void ObtainLists()
        {
            try
            {
				...
				if (CustomPermission.CurrentOperatorRole() == Role.VaultOps)
					status = "Status & " + (int)ReceiveList.GetEligibleStatus(RLStatus.Transit | RLStatus.FullyVerifiedI);
				else  // Don't filter by status
					status = String.Empty;

                RLSorts sortOrder = RLSorts.ListName;
                int pageSize = Preference.GetItemsPerPage();
                receiveLists = ReceiveList.GetReceiveListPage(pageNo, pageSize, sortOrder, status, out listCount);
                ...
                this.ViewState["PageTotal"] = pageTotal;
                // Bind the data
                DataGrid1.DataSource = receiveLists;
                DataGrid1.DataBind();
                // Page links
        ...
        {
            Session[CacheKeys.WaitRequest] = RequestTypes.PrintRLBrowse;
